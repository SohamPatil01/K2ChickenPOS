...
API_PORT=3003 npx pnpm --filter @azela-pos/api dev > api.log 2>&1 &
API_PID=$!
...
NEXT_PUBLIC_API_URL=http://localhost:3003 npx pnpm --filter @azela-pos/hq-web dev > hq-web.log 2>&1 &
HQ_PID=$!
...
curl -s http://localhost:3003/health > /dev/null && echo "✓ API server (3003) is running" || echo "✗ API server (3003) not responding"
curl -s http://localhost:3001 > /dev/null && echo "✓ Web app (3001) is running" || echo "✗ Web app (3001) not responding yet"
